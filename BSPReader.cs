...
public class BSPReader {
	...
	public BSPReader(FileInfo file, mapType readAs) {
		...
		if (!File.Exists(BSPFile.FullName)) {
			Debug.Log("Unable to open BSP file; file "+BSPFile.FullName+" not found.");
		} else {
		...
		try {
			...
			switch(version) {
					...
					for(int i=0;i<64;i++) {
						try {
							switch(i) {
								...
								case 1:
									theLump = readLumpNum(i);
									BSPObject.Planes = CreatePlaneLump(theLump);
									break;
								case 2:
									theLump = readLumpNum(i);
				...
				case mapType.TYPE_SOF:
					Debug.Log("BSP v38 (Quake 2/SiN/Daikatana/Soldier of Fortune)");
					for(int i=0;i<19;i++) {
						try {
							switch(i) {
								case 0:
				...
				case mapType.TYPE_QUAKE:
					Debug.Log("Quake 1/Half-life BSP");
					for (int i = 0; i < 15; i++) {
						try {
							switch (i) {
								case 0:
				...
				case mapType.TYPE_STEF2DEMO:
					Debug.Log("Star Trek Elite Force 2 BSP");
					for (int i = 0; i < 30; i++) {
						try {
							switch (i) {
								case 0:
								...
								case 1:
									theLump = readLumpNum(i);
									BSPObject.Planes = CreatePlaneLump(theLump, 16);
									break;
								case 12:
									theLump = readLumpNum(i);
									BSPObject.BrushSides = BrushSide.createLump(theLump, version);
				...
				case mapType.TYPE_MOHAA: 
					Debug.Log("MOHAA BSP (modified id Tech 3)");
					for(int i=0;i<28;i++) {
						try {
							switch (i) {
								case 0:
				...
				case mapType.TYPE_COD: 
					Debug.Log("BSP v59 (Call of Duty)");
					for(int i=0;i<33;i++) {
						try {
							switch (i) {
								case 0:
				...
				case mapType.TYPE_HEXEN:
					...
					break;
				default:
					Debug.Log("Tried to populate structures for a format not implemented yet!");
					break;
			}
			BSPObject.printBSPReport();
		}
		catch (System.IO.IOException) {
			Debug.Log("Unable to access BSP file! Is it open in another program?");
		}
		br.Close();
	}
	...
	public byte[] readLumpFromHeader(int offset) {
		try {
			...
			Debug.Log("Unknown error reading BSP, it was working before!");
		}
		return new byte[0];
	}
	...
	public static Lump<Plane> CreatePlaneLump(byte[] data) {
		return CreatePlaneLump(data, 20);
	}
	
	public static Lump<Plane> CreatePlaneLump(byte[] data, int structLength) {
		Lump<Plane> lump = new Lump<Plane>(data.Length, structLength, data.Length / structLength);
		byte[] bytes = new byte[structLength];
		int offset = 0;
		for (int j = 0; j < data.Length / structLength; j++) {
			for (int k = 0; k < structLength; k++) {
				bytes[k] = data[offset + k];
			}
			lump.Add(new Plane(DataReader.readPoint3F(bytes[0], bytes[1], bytes[2], bytes[3], bytes[4], bytes[5], bytes[6], bytes[7], bytes[8], bytes[9], bytes[10], bytes[11]), DataReader.readFloat(bytes[12], bytes[13], bytes[14], bytes[15]) * 0.0254f));
			offset += structLength;
		}
		return lump;
	}
