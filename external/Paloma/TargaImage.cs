...
#region NON-COMPRESSED
		...
		private void LoadTGAImage(BinaryReader binReader) {
			...
			if (pf == TextureFormat.BGRA32) {
				...
				pf = TextureFormat.ARGB32;
			}

#if UNITY_5 || UNITY_5_3_OR_NEWER
			bmpTargaImage.LoadRawTextureData(bimagedata);
#else
			bmpTargaImage = LoadRawTextureData(bmpTargaImage, bimagedata);
#endif
				...
				case 32: {
					if (this.Format == TGAFormat.NEW_TGA) {
						switch (this.objTargaExtensionArea.AttributesType) {
							...
							case 2: { // no alpha data
#if UNITY_3_4
								targaPixelFormat = (TextureFormat)14;
#else
...
#else
						bmpImageThumbnail = LoadRawTextureData(bmpImageThumbnail, data);
#endif
					}
				} else {
					UnityEngine.Object.DestroyImmediate(bmpImageThumbnail);
		...
		private void ClearAll() {
			UnityEngine.Object.DestroyImmediate(bmpTargaImage);
			UnityEngine.Object.DestroyImmediate(bmpImageThumbnail);
...
#if !UNITY_5 && !UNITY_5_3_OR_NEWER
		private static Texture2D LoadRawTextureData(Texture2D tex, byte[] data) {
			int offset = 0;
			
			Color32[] pulledColors = new Color32[tex.width * tex.height];
